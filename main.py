...
async def caption(bot, message: Message):
    ...
    if (media is not None) and (media.file_name is not None) and (not message.chat.id in CHANNELS):
        ...
        if Z.__contains__("Fragman") or m.__contains__("Bolum"):
            Jn = m.split("Bolum")[1]
            if "2" in Jn:
                tz += "#دوم"
            elif "1" in Jn:
        ...
        if (not m.__contains__("Bolum") and (N.__contains__("E0") or N.__contains__("E1") or N.__contains__("E2") or N.__contains__("E3") or N.__contains__("E4") or N.__contains__("E5") or N.__contains__("E6") or N.__contains__("E7") or N.__contains__("E8") or N.__contains__("E9")):
            ...
            if not "Hard-Sub" in N:
                ...
                print(Fuc)
                msg = await message.edit(Fuc)
        elif (not m.__contains__("Bolum") and (not N.__contains__("E0") or N.__contains__("E1") or N.__contains__("E2") or N.__contains__("E3") or N.__contains__("E4") or N.__contains__("E5") or N.__contains__("E6") or N.__contains__("E7") or N.__contains__("E8") or N.__contains__("E9")):
            if "20" in D:
                f = D.split("20")[0]
                U = D.split("20")[1]
                K = U.split()[0]
        ...
        if (message.chat.id == -1001457054266) and ((X in NoCopy) or (X in Copy)):
            ...
            if "Duble" in N:
                ...
                if mkv240 and mkv720 and mkv480:
                    if mkv1080:
                        await bot.copy_message(chat_id=kanal, from_chat_id=-1001441684079, message_ids=msgid, caption=kap, parse_mode='markdown')
                        await mkv240p.copy(chat_id=kanal)
                        await mkv480p.copy(chat_id=kanal)
                        await mkv720p.copy(chat_id=kanal)
                        await mkv1080p.copy(chat_id=kanal)
                    else:
                        await bot.copy_message(chat_id=kanal, from_chat_id=-1001441684079, message_ids=msgid, caption=kap, parse_mode='markdown')
                        await mkv240p.copy(chat_id=kanal)
                        await mkv480p.copy(chat_id=kanal)
                    ...
                    if (X in Copy) and (kap2 is not None):
                        await bot.copy_message(chat_id=kanal, from_chat_id=-1001441684079, message_ids=msgid, caption=kap2, parse_mode='markdown')
        ...
        if (message.chat.id == -1001457054266):
            try:
                if "Ghermez" in media.file_name:
                    ...
                    await bot.copy_message(chat_id=-1001457054266, from_chat_id=-1001441684079, message_ids=msgid, caption=kap, parse_mode='markdown')
